...
namespace GraphEditor.Drama
{
    public static class GraphExtension
    {
        ...
        public static GraphNodeDefine GetNode(this GraphNodeDefines nodeDefines, string nodeName)
        {
            if (nodeDefines != null)
            {
                int count = nodeDefines.nodeList.Count;
                for (int i = 0; i < count; ++i)
                {
                    if (nodeDefines.nodeList[i].name == nodeName)
                    {
                        return nodeDefines.nodeList[i];
                    }
                }
            }
            return null;
