...
public class DramaScriptBinaryOpNodeView : Node
{
    ...
    public DramaScriptBinaryOpNodeView()
    {
        this.title = "BinaryOp";
    ...
    private float GetInputValue(Port port)
    {
        if (port.node != null)
        {
            foreach (Edge connection in port.connections)
            {
                ...
                {
                    DramaScriptFloatNodeView floatNode = connection.output.node as DramaScriptFloatNodeView;
                    if (floatNode != null)
                    {
